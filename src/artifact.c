...
int tmp;
{
	...
	if (spec_dbon_applies){
		...
		} else if(otmp && otmp->oartifact == ART_LIMITED_MOON || otmp->oartifact == ART_POSEIDON_S_TRIDENT){
	...
	*dmgptr += spec_dbon(otmp, mdef, *dmgptr);
	...
	if (attacks(AD_DRIN, otmp)) {
		...
		if(spec_dbon_applies && realizes_damage && otmp->oartifact != ART_ILLITHID_STAFF)
			if(cancel_monst(mdef, otmp, youattack, FALSE, FALSE,0)) pline_The("%s %s %s %s%c", "sparkling", otmp->oartifact==ART_ELDER_CEREBRAL_FLUID?"crystal":"blade",
				"cancels",
				hittee, '!');
		messaged = realizes_damage;
	}
    ...
    register struct obj *obj;
{
   ...
   if(!oart || !oart->inv_prop) {
	...
	case SHATTER_TOGGLE:{
		if(obj->ovar1) pline("%s will now shatter weapons.",The(xname(obj)));
		else pline("%s will no longer always shatter weapons.",The(xname(obj)));
		obj->ovar1 = !obj->ovar1;
		break;
	}
	case HEALING: {
    ...
    struct obj *obj;
{
    return	(get_artifact(obj) && 
				(obj->oartifact == ART_SUNSWORD ||
				 ...
				 obj->oartifact == ART_JACK_S_TORCH ||
