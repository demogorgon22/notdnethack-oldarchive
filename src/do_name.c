...
const char *name;
{
	...
	if (obj->oartifact) {
		...
		else if(obj->oartifact == ART_PIRATE_KING_S_CROWN) obj->obj_material = GOLD;
		...
		else if(obj->oartifact == ART_STONE_MASK) obj->obj_material = MINERAL;
		else if(obj->oartifact == ART_ARCHIVIST) obj->obj_material = GEMSTONE;
		else if(obj->oartifact == ART_FLICKERING_OUTLINE) obj->obj_material = SHADOWSTUFF;
		else if(obj->oartifact == ART_CROWN_OF_BERITH) obj->obj_material = GOLD;
...
boolean called;
{
	...
	if (mtmp->isshk && !do_hallu) {
		...
		else if(mtmp->mfaction == WHISPERING) Strcat(buf, "'s whispers");
	    return buf;
	}
	...
	if (do_hallu) {
	    ...
	    if (mdat == &mons[PM_GHOST]) {
			...
			if(type_is_pname(mdat)){
				...
				else if(mtmp->mfaction == WHISPERING) Strcat(buf, " of whispers");
			}
